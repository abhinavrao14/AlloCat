...
namespace Allocat.CustomService.Authorize.Net
{
    ...
    public class APICalls : AllocatCustomService
    {
        ...
        public ResTransaction ChargeCreditCard(Customer objCustomer, decimal AmountToBeCharged)
        {
            ResTransaction objResTransaction = null;
            try
            {
                logEntry = new LogEntry { Message = "Call ChargeCreditCard Method....", Severity = TraceEventType.Information };
                WriteLog(logEntry);
                // Environment Settings 
                if (objConfigurationReader.Environment.ToUpper() == "SANDBOX")
                {
                ...
                var creditCard = new creditCardType
                {
                    cardNumber = objCustomer.CardInfo.CreditCardNumber,
                    expirationDate = objCustomer.CardInfo.ExpiryDate,
                    cardCode = objCustomer.CardInfo.CardCode
                };
                ...
                var billingAddress = new customerAddressType
                {
                    firstName = objCustomer.FirstName,
                    lastName = objCustomer.LastName,
                    address = objCustomer.HomeAddress.Address,
                    city = objCustomer.HomeAddress.City,
                    zip = objCustomer.HomeAddress.ZipCode
                };
                ...
                {
                    itemId = objConfigurationReader.ServiceFeeItemId,
                    name = objConfigurationReader.ServiceFeeItemName,
                    quantity = objConfigurationReader.ServiceFeeQuantity,
                    unitPrice = (objConfigurationReader.ServiceFeeUnitPrice * AmountToBeCharged)
                };
